...
def chat(agent_path: Path, workspace: Path | None, model: str | None):
    """Interactive chat with an agent package."""
    asyncio.run(_chat(agent_path, workspace, model))
...
def run(agent_path: Path, workspace: Path | None, model: str | None, message: str):
    """Single-shot run. Prints the agent response."""
    asyncio.run(_run_once(agent_path, workspace, model, message))
...
def _harness(agent_path: Path, workspace: Path | None, model: str | None) -> Harness:
    pkg = load_package(agent_path, workspace_override=workspace)
    try:
        llm = create_llm(provider=pkg.provider, model=model or pkg.model)
...
async def _chat(agent_path: Path, workspace: Path | None, model: str | None):
    from agentgw.harness.session import Session

    harness = _harness(agent_path, workspace, model)
...
async def _run_once(agent_path: Path, workspace: Path | None, model: str | None, message: str):
    harness = _harness(agent_path, workspace, model)
